...
export async function seed(knex: Knex): Promise<void> {
    ...
    const randomBody = () => {
        return "I really liked that this class review contains the number " + _.random(1000); 
    }
    ...
    courses.forEach( course => {
        for (let i = 0; i < _.random(16); i++) {
            ...
            if (!user.last_signed_in || typeof user.last_signed_in === 'string') user.last_signed_in = timestamp;
